...
export default function RpCardVisual({ holderName, accountNumberDisplay, lastFour }: Props) {
  return (
    <div className="relative mx-auto w-full max-w-[420px]">
      <div className="absolute -inset-4 rounded-[2rem] bg-purple-400/20 blur-2xl" aria-hidden />
      <div
        className="relative aspect-[1.586/1] overflow-hidden rounded-2xl text-white shadow-2xl"
        style={{
          background:
            'linear-gradient(145deg, #2e1065 0%, #4c1d95 35%, #5b21b6 70%, #6d28d9 100%)',
        }}
      >
        <div className="absolute inset-0 opacity-30 bg-[radial-gradient(circle_at_15%_20%,#a78bfa_0%,transparent_45%)]" />
        <div className="absolute -right-8 -top-8 h-40 w-40 rounded-full bg-white/5" />
        <div className="absolute -bottom-12 -left-8 h-48 w-48 rounded-full bg-purple-400/10" />

        <div className="relative flex h-full flex-col justify-between p-6 sm:p-8">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-purple-300">R&P Global</p>
              <p className="font-display text-xl font-bold tracking-tight">Energies</p>
            </div>
            <div className="h-9 w-12 rounded-md bg-purple-shine shadow-inner" />
          </div>
